...
public class AdminProduct {
	 ...
	 * 
	 * if (image != null && !image.isEmpty()) { try { String rootDirectory =
	 ...
	 * System.out.println(path.toString()); image.transferTo(new
	 * File(path.toString())); } catch (Exception e) { e.printStackTrace();
	 ...
	 * request.getSession().getServletContext().getRealPath("/"); path =
	 * Paths.get(rootDirectory + "WEB-INF/resources/images/" +
	 * product.getProductId() + ".jpg"); image.transferTo(new
	 * File(path.toString())); } catch (Exception e) { e.printStackTrace();
	 * throw new RuntimeException("Error occured while saving image", e); }
	 * }
	 */
	 ...
	 * request.getSession().getServletContext().getRealPath("/"); path =
	 * Paths.get(rootDirectory+"WEB-INF/resources/images/"+productId+".jpg")
	 * ; try { Files.deleteIfExists(path); } catch (IOException e) { // TODO
